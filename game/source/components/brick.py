...
def create_brick(brick_group, item, level):
    ...
    if type == c.TYPE_COIN:
        ...
        type == c.TYPE_STAR or type == c.TYPE_FIREFLOWER or type == c.TYPE_LIFEMUSHROOM
    ):
        brick_group.add(Brick(x, y, type, color, level.powerup_group))
    else:
        if c.BRICK_NUM in item:
            create_brick_list(
                brick_group, item[c.BRICK_NUM], x, y, type, color, item["direction"]
            )
        else:
...
class Brick(stuff.Stuff):
    def __init__(self, x, y, type, color=c.ORANGE, group=None, name=c.MAP_BRICK):
        ...
        stuff.Stuff.__init__(
            self, x, y, setup.GFX["tile_set"], frame_rect, c.BRICK_SIZE_MULTIPLIER
        )
    ...
    def bumped(self):
        ...
        if self.rect.y >= self.rest_height:
            ...
            if self.type == c.TYPE_COIN:
                ...
                self.state = c.OPENED
                self.group.add(
                    powerup.LifeMushroom(self.rect.centerx, self.rest_height)
                )
            else:
                self.state = c.RESTING
    ...
    def start_bump(self, score_group):
        ...
        if self.type == c.TYPE_COIN:
            ...
            or self.type == c.TYPE_FIREFLOWER
            or self.type == c.TYPE_LIFEMUSHROOM
        ):
            self.frame_index = 1
            self.image = self.frames[self.frame_index]
...
class BrickPiece(stuff.Stuff):
    def __init__(self, x, y, x_vel, y_vel):
        stuff.Stuff.__init__(
            self, x, y, setup.GFX["tile_set"], [(68, 20, 8, 8)], c.BRICK_SIZE_MULTIPLIER
        )
        self.x_vel = x_vel
        self.y_vel = y_vel
        self.gravity = 0.8
